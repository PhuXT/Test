...
function findIDByName(fullName, arr) {
    let arr2 = fullName.split(' ')
    let value =  arr.find( user => {
        return user.firstName == arr2[0] && user.lastName == arr2[1]
    })
    return value.id
}

function findIDByName2(target, arr) {
    arr.forEach(user => {
...
// console.log(findIDByName('Madison Marshall', users));
// findIDByName2('Madison Marshall', users)

// 2) Who wrote the first comment (assuming the first comment is in position 0 of the comments array)

let firstUserComment =  users.find( user => {
    return comments[0].userId == user.id
})
// console.log(firstUserComment);
...
comments.map(commnet => {
    let u = users.find( (user => {
        return user.id == commnet.userId
    }))
    return commnet.fullName = u.firstName +' ' + u.lastName 
})
